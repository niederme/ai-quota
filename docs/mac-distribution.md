...
| --- | --- | --- | --- |
| Website / GitHub release | `AIQuota.xcodeproj` / `AIQuota` | Sparkle, existing feed | Existing unrestricted Mac app |
| TestFlight / Mac App Store | `AIQuota.xcodeproj` / `AIQuota-TestFlight` | Apple | App and widget sandboxed with outgoing network access |
...
Generate the project when changing its spec, then select `AIQuota-TestFlight` and Product → Archive in Xcode beta:
...
Distribute the resulting archive through App Store Connect in Organizer. Do not reuse an archive built with the regular `AIQuota` scheme: its app and Sparkle executables are not the TestFlight distribution configuration.
...
DEVELOPER_DIR=/Applications/Xcode-beta.app/Contents/Developer xcodebuild \
  -project AIQuota.xcodeproj -scheme AIQuota -configuration Release \
